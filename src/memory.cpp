...
#error TODO
	...
	std::vector<Ptr> memory::signed_pattern_scan_region(MemoryRegion region, const SignedBytePattern& pattern) {
		...
		while (remaining_size >= min_size) {
			...
			{ // Check pattern
				...
				if (matches) {
					results.push_back(next_possible_base.as_addr() - leading_wildcards);
				}
			}

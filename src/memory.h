...
namespace ztour::memory {
	...
	Ptr signed_pattern_scan_region(MemoryRegion region, const SignedBytePattern& pattern);
	Ptr pattern_scan_module(const std::string& module_name, const std::string& pattern_str);
}